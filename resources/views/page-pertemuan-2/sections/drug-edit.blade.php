@php
    $jenis_collection = ['Tablet', 'Cair', 'Salep'];
    $satuan_collection = ['g', 'ml', 'mg'];
@endphp
...
@section('content')
    ...
    <div class="py-10 bg-white w-full">
        <form action="{{ route('drug.update', $drug->drug_id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="relative z-0 w-full mb-5 group">
                <input type="nama" name="nama" id="nama" autocomplete="off"
                    ...
                    placeholder=" " value="{{ $drug->nama }}" required />
                <label for="nama"
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select id="jenis" name="jenis" autocomplete="off" required
                        ...
                        @foreach ($jenis_collection as $item)
                            <option {{ $item == $drug->jenis ? 'selected' : '' }}>{{ $item }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select id="satuan" name="satuan" autocomplete="off" required
                        class="bg-white text-gray-500 text-sm focus:outline-none focus:ring-0 block w-full">
                        @foreach ($satuan_collection as $item)
                            <option {{ $item == $drug->satuan ? 'selected' : '' }}>{{ $item }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="flex justify-end">
                <a type="button" id="close_modal" href="{{ route('drug') }}"
